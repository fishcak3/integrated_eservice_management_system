        ...
        <form method="GET" action="{{ url()->current() }}" x-on:change="$el.submit()" class="flex flex-col gap-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                @if(request('statuses') || request('sectors') || request('sitios') || request('is_family_head') || request('search'))
                    <flux:button href="{{ route('official.residents.index', ['type' => request('type')]) }}" size="sm" variant="subtle" icon="x-mark">
                        Clear
                    </flux:button>
                @endif
            </div>
                ...
                <flux:table.rows>
                    @forelse ($residents as $resident)
                        ...
                        <flux:table.row 
                            ...
                            onclick="window.location.href='{{ route('official.residents.show', $resident->id) }}'"
                        >
                            {{-- Row Checkbox --}}
