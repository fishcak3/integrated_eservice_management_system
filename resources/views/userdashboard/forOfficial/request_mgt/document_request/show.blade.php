    ...
    <div class="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
        ...
        <div class="flex items-center gap-2">
            ...
            @if($documentRequest->status === 'pending')
                <flux:button href="{{ route('official.documents.process', $documentRequest->id) }}" variant="primary" icon="play">
                    Process
                </flux:button>

                <form action="{{ route('official.documents.update-status', $documentRequest->id) }}" method="POST" class="inline">
                    @csrf
                    @method('PATCH')
            ...
            @elseif($documentRequest->status === 'processing')
                ...
                <div class="flex items-center gap-2 bg-zinc-50 dark:bg-zinc-800/50 p-2 rounded-lg border border-zinc-200 dark:border-zinc-700">
                    ...
                    <form action="{{ route('official.documents.request-e-sign', $documentRequest->id) }}" method="POST" class="inline">
                        @csrf
                ...
                {{-- Reject Button --}}
                <form action="{{ route('official.documents.update-status', $documentRequest->id) }}" method="POST" class="inline ml-2">
                    @csrf
                    @method('PATCH')
                    <input type="hidden" name="status" value="rejected">
                    <flux:button type="submit" variant="danger" icon="x-circle">
           ...
           {{-- 3. PENDING E-SIGNATURE STATE ACTIONS --}}
            @elseif($documentRequest->status === 'pending_e_signature')
                ...
                @if(auth()->user()->isCurrentOfficialPosition('Barangay Captain'))
                    <form action="{{ route('official.documents.approve-sign', $documentRequest->id) }}" method="POST" class="inline">
                        @csrf
                        @method('PATCH')
                ...
                {{-- If the logged-in user is an Official but NOT the Captain --}}
                @else
            ...
            <flux:button href="{{ route('official.documents.preview', $documentRequest->id) }}" target="_blank" variant="subtle" icon="eye">
                Preview
            </flux:button>
            <flux:button href="{{ route('official.documents.edit', $documentRequest->id) }}" variant="subtle" icon="pencil">Edit</flux:button>
        </div>
    </div>
                ...
                <flux:card class="p-6">
                    @if($documentRequest->user)
                        ...
                        @if($residentProfile)
                            ...
                            <div class="mt-6">
                                {{-- Assuming Officials also use this same route to view residents, if not adjust to official.residents.show --}}
