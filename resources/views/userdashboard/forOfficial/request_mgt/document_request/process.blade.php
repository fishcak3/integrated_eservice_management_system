<x-layouts::app :title="__('Process Document')">
    ...
    <form action="{{ route('official.documents.update', $documentRequest->id) }}" method="POST" class="space-y-10">
        @csrf
        @method('PUT')
        ...
        <div class="flex flex-col-reverse sm:flex-row justify-end items-center gap-4 pt-6 border-t border-zinc-200 dark:border-zinc-800">
            <flux:button href="{{ route('official.documents.show', $documentRequest->id) }}" variant="subtle">Cancel</flux:button>
            
            <flux:button 
                href="{{ route('official.documents.preview', $documentRequest->id) }}" 
                target="_blank" 
                variant="outline" 
