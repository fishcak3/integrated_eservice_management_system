<x-layouts::app :title="__('Complaints Management')">
    ...
    <div x-data="{
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <flux:button href="{{ route('official.complaints.create') }}" variant="primary" icon="plus">File Complaint</flux:button>
        </div>
        ...
        <form method="GET" action="{{ url()->current() }}" x-on:change="if (!$event.target.hasAttribute('data-no-submit')) $el.submit()" wire:key="filter-form-complaints" class="flex flex-col gap-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                @if(request()->hasAny(['search', 'severities', 'complaint_statuses', 'date_from', 'date_to']))
                    <flux:button href="{{ route('official.complaints.index') }}" size="sm" variant="subtle" icon="x-mark">
                        Clear
                    </flux:button>
                @endif
            </div>
                ...
                <flux:table.rows>
                    @forelse($complaints as $case)
                        <flux:table.row 
                            ...
                            onclick="window.location.href='{{ route('official.complaints.show', $case->id) }}'"
                        >
                            ...
                            <flux:table.cell align="end" onclick="event.stopPropagation()">
                                <flux:dropdown>
                                    ...
                                    <flux:menu>
                                        <flux:menu.item href="{{ route('official.complaints.show', $case->id) }}" icon="eye">
                                            View Details
                                        </flux:menu.item>
                                    </flux:menu>
                                </flux:dropdown>
