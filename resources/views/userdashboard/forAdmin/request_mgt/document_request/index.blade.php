<x-layouts::app :title="__('Document Requests')">
    ...
    <div x-data="{
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <flux:button href="{{ route('admin.documents.create') }}" variant="primary" icon="plus">New Request</flux:button>
        </div>
        ...
        <form method="GET" action="{{ url()->current() }}" x-on:change="if (!$event.target.hasAttribute('data-no-submit')) $el.submit()" wire:key="filter-form-documents" class="flex flex-col gap-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                @if(request()->hasAny(['search', 'doc_types', 'statuses', 'date_from', 'date_to']))
                    <flux:button href="{{ route('admin.documents.index') }}" size="sm" variant="subtle" icon="x-mark">
                        Clear
                    </flux:button>
                @endif
            </div>
                ...
                <flux:table.rows>
                    @forelse($documentRequests as $req)
                        <flux:table.row 
                            ...
                            onclick="window.location.href='{{ route('admin.documents.show', $req->id) }}'"
                        >
                            ...
                            <flux:table.cell align="end" onclick="event.stopPropagation()">
                                <flux:dropdown>
                                    ...
                                    <flux:menu>
                                        <flux:menu.item href="{{ route('admin.documents.show', $req->id) }}" icon="eye">
                                            View Details
                                        </flux:menu.item>

                                        <flux:menu.item href="{{ route('admin.documents.edit', $req->id) }}" icon="pencil-square">
                                            Edit
                                        </flux:menu.item>

                                        @if($req->status === 'pending')
                                            <flux:menu.separator />
                                            <flux:menu.item href="{{ route('admin.documents.process', $req->id) }}" icon="arrow-path">
                                                Process Request
                                            </flux:menu.item>
                                        @endif
