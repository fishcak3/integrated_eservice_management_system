<x-layouts::app :title="__('Manage Complaint')">
    ...
    <div class="space-y-10">
        ...
        {{-- Section 2: Case Assignment --}}
        <div class="grid grid-cols-1 gap-x-8 gap-y-8 first:pt-0 md:grid-cols-3">
            <div class="px-4 sm:px-0">
                <flux:heading size="lg">Case Assignment</flux:heading>
                <flux:text variant="subtle" class="mt-1">
                    Assign an official to oversee and manage this specific case.
                </flux:text>
            </div>

            <div class="md:col-span-2">
                <flux:card class="p-6">
                    <form action="{{ route('admin.complaints.assign', $complaint->id) }}" method="POST" class="flex flex-col gap-4 sm:flex-row sm:items-end">
                        @csrf
                        <div class="w-full flex-1">
                            <flux:label>Assigned Official</flux:label>
                            <flux:select name="official_id">
                                <option value="">Unassigned (Select an admin...)</option>
                                @foreach($officials as $official)
                                    <option value="{{ $official->id }}" @selected($complaint->assigned_official_id == $official->id)>
                                        {{ $official->name }}
                                    </option>
                                @endforeach
                            </flux:select>
        ...
        <div class="grid grid-cols-1 gap-x-8 gap-y-8 first:pt-0 md:grid-cols-3">
            ...
            <div class="md:col-span-2">
                <flux:card class="p-6">
                    <form action="{{ route('admin.complaints.update-status', $complaint->id) }}" method="POST" x-data="{ currentStatus: '{{ $complaint->status }}' }" class="flex flex-col gap-6">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="space-y-4 rounded-lg bg-zinc-50 p-5 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700">
                            ...
                            {{-- Always Show Admin Remarks --}}
                            <div>
                                <flux:textarea name="admin_remarks" label="Admin Remarks / Update Log" rows="2" placeholder="Brief note about this update (optional)..."></flux:textarea>
                            </div>
                        </div>
