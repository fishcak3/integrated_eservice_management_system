<x-layouts::app :title="__('Household Management')">
        ...
        <form method="GET" action="{{ url()->current() }}" x-on:change="$el.submit()" class="flex flex-col gap-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                @if(request('sitios') || request('members') || request('search'))
                    <flux:button href="{{ route('admin.residents.household', ['type' => request('type')]) }}" size="sm" variant="subtle" icon="x-mark">
                        Clear
                    </flux:button>
                @endif
            </div>
        ...
        <div class="border border-zinc-200 dark:border-zinc-700 rounded-xl overflow-hidden">
            <flux:table class="whitespace-nowrap bg-transparent">
                <flux:table.columns>
                    ...
                    <flux:table.column x-show="cols.id">House Number</flux:table.column>
                ...
                <flux:table.rows>
                    @forelse ($households as $household)
                        <flux:table.row 
                            ...
                            onclick="window.location.href='{{ route('admin.residents.household.show', $household->id) }}'"
                        >
                            {{-- Row Checkbox --}}
                            ...
                            <flux:table.cell align="end"  onclick="event.stopPropagation()">
                                <flux:dropdown>
                                    ...
                                    <flux:menu>
                                        <flux:menu.item href="{{ route('admin.residents.household.show', $household->id) }}" icon="eye">View</flux:menu.item>
                                        <flux:menu.separator />
    ...
    {{-- Modals --}}
    @foreach ($households as $householdModal)
        <x-delete-modal 
            name="delete-household-{{ $householdModal->id }}" 
            action="{{ route('admin.residents.household.destroy', $householdModal->id) }}"
        >
            This will permanently delete the household profile for <strong>Household #{{ $householdModal->household_number }}</strong>. This action cannot be undone.
        </x-delete-modal>
    @endforeach
